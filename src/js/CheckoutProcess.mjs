...
export default class CheckoutProcess {
    ...
    calculateItemSubTotal() {
        ...
        const elemento = document.querySelector(`${this.outputSelector} #subtotal`);
        if (elemento) {
            elemento.innerText = `$${this.itemTotal.toFixed(2)}`;
        }

        // Mostrar cantidad de items
        const contador = document.querySelector(`${this.outputSelector} #itemCount`);
        if (contador) {
            contador.innerText = this.list.length;
        }
    }

    calculateOrderTotal() {
        // calculate the tax and shipping amounts. Add those to the cart total to figure out the order total
        this.tax = this.itemTotal * 0.06; // 6% sales tax on subtotal
